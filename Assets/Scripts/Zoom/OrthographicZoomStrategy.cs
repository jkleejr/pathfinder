...
public class OrthographicZoomStrategy : IZoomStrategy // controlling camera
{
    // create izoomstrategy at runtime
    ...
    {
        if (cam.orthographicSize == nearZoomLimit) return; // already zoomed in fully, otherwise
        cam.orthographicSize = Mathf.Max(cam.orthographicSize - delta, nearZoomLimit); // stay within zoomlimit bounds
    }
